...
SELECT sex, education, ROUND(AVG(credit_amount), 1) AS 'avg_credit', AVG(income) AS 'avg_income' FROM Clusters c 
WHERE is_client = 1
...
*/
SELECT sex, bad_client_target, MIN(credit_amount), MAX(credit_amount) FROM Clusters c 
...
*/
SELECT country, COUNT(*) AS cnt FROM Customers c
GROUP BY Country
ORDER BY cnt DESC 
LIMIT 3;
